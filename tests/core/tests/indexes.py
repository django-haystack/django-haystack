...
class MROFieldsSearchIndexA(SearchIndex):
    text = CharField(document=True, model_attr='test_a')


class MROFieldsSearchIndexB(SearchIndex):
    text = CharField(document=True, model_attr='test_b')
...
class SearchIndexTestCase(TestCase):
    ...
    def test_proper_field_resolution(self):
        mrofsc = MROFieldsSearchChild(MockModel, backend=self.msb)
        mock = MockModel()
        mock.pk = 20
        mock.author = 'daniel%s' % mock.id
        mock.pub_date = datetime.datetime(2009, 1, 31, 4, 19, 0)
