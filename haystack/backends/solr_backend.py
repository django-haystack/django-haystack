...
class SolrSearchBackend(BaseSearchBackend):
    ...
    def search(self, query_string, sort_by=None, start_offset=0, end_offset=None,
        ...
        if facets_kwargs is not None:
            for facet_field, facet_params in facets_kwargs.items():
                for key, value in facet_params.items():
                    kwargs["f.%s.facet.%s" % (facet_field, key)] = value
...
class SolrSearchQuery(BaseSearchQuery):
    ...
    def run(self, spelling_query=None, **kwargs):
        ...
        if self.facets_kwargs:
            search_kwargs['facets_kwargs'] = self.facets_kwargs
