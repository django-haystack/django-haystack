...
def haystack_get_models(label):
    try:
        ...
        # Sort the models by name, so the caller gets a consistent order
        return sorted(app_mod.get_models(), key=lambda model: (model._meta.app_label, model._meta.object_name))
    except LookupError:
        if '.' not in label:
