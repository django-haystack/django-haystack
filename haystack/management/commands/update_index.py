...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        if not self.apps:
            ...
            app_args = self.apps.split(',')

            for app in app_args:
                app_split = app.split('.')
                if len(app_split) > 1:
                    app_label = app_split[-2]
                    model = app_split[-1]
                else:
                    app_label = app_split[-1]
                    model = '*'
        ...
        for app in app_list:
            ...
            if model == '*':
                ...
                try:
                    models = [getattr(loaded_app, model)]
                except AttributeError:
                    raise CommandError(
                        'No model named "%s" could be found in app "%s" (note: model name is case sensitive)' % (
                        model, app_label
                    ))

            self.index_models(models)

    def index_models(self, models):
